...
def build_lag_matrix(
    ...
    *,
    ...
    """Build lagged design matrix for a 1D stimulus."""
    ...
    n_cols = n_lags + (1 if add_intercept else 0)

    x_mat = np.empty((n_rows, n_cols), dtype=float)
    col = 0
    if add_intercept:
        x_mat[:, 0] = 1.0
        col = 1
...
def predict_rate(x_mat: ArrayF | np.ndarray, coef: ArrayF | np.ndarray) -> ArrayF:
    """Predict rate with exponential nonlinearity (log link)."""
    eta = x_mat @ coef
...
def fit_lnp_poisson_irls(
    ...
    *,
    ...
    """Fit an LNP model with Poisson likelihood using IRLS.
    ...
    alpha:
        L2 regularization strength (ridge). Applied to non-intercept terms.
    max_iter:
    ...
    x_mat, valid_idx = build_lag_matrix(stim, lags, add_intercept=add_intercept)
    y_valid = y[valid_idx]
    ...
    require_non_negative_scalar(alpha, name="alpha")
    ...
    return LNPResult(coef=coef, se=se, log_likelihood=ll, n_iter=n_iter, converged=converged)


__all__ = [
    "LNPResult",
    "build_lag_matrix",
    "fit_lnp_poisson_irls",
    "predict_rate",
